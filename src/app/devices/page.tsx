...
export default function DevicesPage() {
  ...
  const deviceListFiltered = devices.filter((x) =>
  ...
  return (
    <div className="flex flex-col space-y-10 justify-center items-center">
      ...
      <div className="lg:w-2/3 md:w-3/5 w-full flex items-center justify-center">
        <Filter value={nameFilter} setValue={(e: string) => setNameFilter(e)} placeholder="Search for device" searchOptions={(<p className="text-sm text-gray-500">{deviceListFiltered.length} items founded</p>)}/>
      </div>
